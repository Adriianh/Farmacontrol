...
{
    public static class CartPrinterComponent
    {
        public static void PrintCart(Sale sale, decimal discountPercent, bool applyTax)
        {
            ...
            else
            {
                ...
                {
                    var pName = detail.ProductName.Length > 28
                        ? detail.ProductName.Substring(0, 28) + ".."
                        : detail.ProductName;
                    Console.WriteLine(
                        $"{detail.ProductCode,-8} | {pName,-30} | Q{detail.UnitPrice,8:F2} | {detail.Quantity,4} | Q{detail.Subtotal,8:F2}");
                }
            }
            ...
            var taxAmt = applyTax ? (subtotal - discountAmt) * 0.12m : 0;
